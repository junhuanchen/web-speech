(function (window) {
	...
	var HZRecorder = function (stream, config) {
		...
		var isrunning = false;
		var volumeLimit = 30; // 与音量比较，停止录音输入的判定阀值，与下结合
		var volumeStop = 20; // 录音停止持续阀值，值越小，就越容易判定为录音输入结束
		...
		recorder.onaudioprocess = function (e) {
			...
			console.log(vol);
			if (vol < volumeLimit) { //设置音量停止阀值  数值越大越容易停
				emptydatacount++;
				console.log(emptydatacount);
				if (emptydatacount > volumeStop) { //设置静音停止次数
					self.recorder.stop();
					console.log('stoped');
				}
			} else {
